...
The global initiative project e-WindLidar focused on development of wind lidar community based tools and data standards, and published a lidar data form to make them Findable, Accessible, Interoperable and Reusable (FAIR) \cite{ref-FAIR}; 
...
The OpenLidar project \cite{clifton_andrew_2019_3414197} -- coordinated by the Stuttgart Wind Energy -- developed an architecture of an open-source remote wind sensing lidar that can be used for teaching, research, and product development \cite{clifton_andrew_2019_3414197}.
...
The wind lidar ontology \cite{ref-Clifton-Costa} was created as a subtask of the IEA Wind TCP Task 32, and has been taken on by a group of academics and industry engineers.
...
% DL rephrased
This study employs a controlled vocabulary approach, which entails the use of a defined set of terms and phrases pertaining to a specific knowledge domain. The chosen terms are employed to tag units of information, facilitating their retrieval during a search. The metadata is structured according to this controlled vocabulary, enabling a clear representation of our data sets and fostering consistency in data storage, maintenance, and sharing. Each term within this controlled vocabulary serves as a concept within the specified knowledge domain. 
...
% DL rephrased
The Resource Description Framework (RDF), a standard model for data interchange on the Web, is used to model the controlled vocabulary and define the properties, relationships, constraints, and axioms among its concepts \cite{ref-W3C-RDF}. Concurrently, we utilize the Simple Knowledge Organization System (SKOS) to establish data models for metadata \cite{ref-W3C-SKOS}. The combination of RDF and SKOS provides a standardized data organization model that is machine-readable. 

\subsection{Taxonomy and ontology}
A taxonomy is a type of controlled vocabulary in which concepts are related in a hierarchical order or sorted into categories. An ontology is a type of controlled vocabulary which identifies and distinguishes concepts and their relationships.
...
\begin{description}
    ...
    \item [OntoStack:] The Ontostack framework is a set of tools combined for the purpose of handling vocabularies and their RDF properties, as such yielded by sheet2rdf, and render them in human- and machine-readable formats, and facilitates the deployment and visualisation of the wind lidar ontology.
...
\hline
\textbf{Definition}	& Consensus definition --- translated to several languages --- describing shortly the ontology concept and its role in the lidar field\\
\hline
...
In the Methodology section, we discussed the process of how we established the wind lidar concepts and terminology based on the feedback from participants from different fields working in wind energy. During these discussions, we developed some tools to ease the application of metadata on data sets and allow standardisation across different application domains in wind energy. The Ontology tool is based on tools developed by the FAIR Data Collective discussed in sub-section \ref{subsec:implementation}. The Ontology tool is openly available at the IEA Wind TCP Task 52 Github repository: \href{https://github.com/PacoCosta/Extract-lidar-ontology-concepts}{https://github.com/PacoCosta/Extract-lidar-ontology-concepts} and we demonstrate an example using this tool here.

In order to merge the Ontology with any application, there are two alternatives. In the first option, one can download specific keywords from the webserver-based Ontology database in file formats like RDF, Turtle or JSON. In the second option, a database file already existing in the Github repository can be accessed and filtered through any programming tool (e.g. Python) for specific keywords. The steps 1,2,3,4 can be set accordingly in the latter case. We describe the first step in detail here, as this is more exhaustive and provides a better overview of the steps involved.
...
\newpage
 ...
 Using the code snippets shown in the next code blocks, the data is fetched from the Ontology files in a specific language (e.g. English). The code snippets are numbered according to the workflow in Figure \ref{Ontology_workflow}, which is divided into the following elements:
...
\begin{enumerate}
    ...
    \item Extract the desired information from the ontology. This information is available in different languages.
    \item Store the lidar concept and its fields of interest as a python dictionary 
    \item Apply the output (metadata parameters e.g. definition, preferred Label, uri, etc.) to your application.
\end{enumerate}

The \textit{Ontology Viewer} is a webserver where the definitions are stored and is hosted by DTU. The block \textit{Download an Ontology concept} refers to a user action of searching a keyword, selecting the definition and downloading this definition from the Ontology Viewer. This process is shown in the block below. 

\begin{tcolorbox}
    Workflow block 2:  Download an Ontology concept
\begin{python}
...
The block \textit{Github Tool} refers to the python tool developed to extract data from the downloaded definition and making it available for further use. The block \textit{Backed Functions} refers to the python routines that help the user in fetching definitions. The sequence followed by the workflow is shown in the code below. 
...
Finally, the block \textit{Output} refers to the output from the fetched data that describes the definition of the Ontology's variable. It is a dict-like Python object, which includes uri, type, alternative and preferred labels, definitions and editorial notes. 
...
YAML, for Yet Another Markup Language, is a human-friendly, machine-readable data serialisation language commonly used for software configuration files. As an application, the next code block shows how a local YAML file, used as an input file for a lidar simulator, is updated with the information extracted from the ontology. By selecting the concept (\texttt{tag}) and the fields we want to edit (\texttt{fields2change}), the fetched information is incorporated into the local file. The updating process avoids overwriting, preserving any information contained in the local YAML file. 
...
reference wind turbines (IEA Wind TCP Task 37), wind energy digitalisation (IEA Wind TCP Task 43). Produce collaborative domain ontologies especially dedicated to remote sensing science.
...
\dataavailability{The Lidar ontology viewer is available at:  \href{https://data.windenergy.dtu.dk/ontologies/view/ontolidar/en/}{Lidar ontology viewer}. The GitHub tool is available on github under: \href{https://github.com/PacoCosta/Extract-lidar-ontology-concepts}{GitHub tool}.} 
